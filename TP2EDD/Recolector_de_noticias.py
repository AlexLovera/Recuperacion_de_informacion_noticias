...
def _compara_xmls_y_añade_articulos_nuevos(url,xmlfile):
    ...
    #####################################
        ...
        if "not well-formed" in e.__str__():
            print(color_error+"Estructura de la pagina mal formada o pagina caida")
        print(url+color_blanco)

def _crear_xml_mediante_config_y_agrega_articulo(config):
    ...
    for medio in configuracion.sections():
        ...
        numero_de_tupla = 0
        #tengo que ignorar las primeras tuplas? son items del default
        for tupla_de_valores in lista_de_tupla_items_de_la_seccion:
            ...
            ## for variable,valor in tupla_de_valores: parece que no funciona en tuplas
            if(numero_de_tupla<=4):
                continue#pasa los valores por defecto
            if numero_de_tupla>4:
                seccion=str(valor)
                # print(f"Medios/{medio}/{str(variable) + '.xml'}")
                # print(not os.path.exists(f"Medios/{medio}/{str(variable) + '.xml'}"))
            # print(url_base + seccion)#compruebo que imprima bien el url
            # if not os.path.isdir(f"Medios/{medio}"):#PARA CREAR LA CARPETA PARA EL MEDIO SI NO EXISTE
            #     print('Creando carpeta para el medio');
            #     os.mkdir(f"Medios/{medio}")
            if not os.path.exists(f"Medios/{medio}/{str(variable) + '.xml'}"):#para saltear los valor default y url
                ...
                #no hacer nada? ejecutar el programa de recoleccion?
                # print(f"Medios/{medio}/{str(variable) + '.xml'}")
                _compara_xmls_y_añade_articulos_nuevos(url_base + seccion,f"Medios/{medio}/{str(variable) + '.xml'}")

def recolectar_articulos_segun_config(config):
    try:
        _crear_xml_mediante_config_y_agrega_articulo(config)
